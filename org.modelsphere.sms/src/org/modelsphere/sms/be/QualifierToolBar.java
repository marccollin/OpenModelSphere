...
public class QualifierToolBar extends JackToolBar implements CurrentFocusListener,
        DbRefreshListener {
    public static final String KEY = "ToolbarQualifier"; // NOT LOCALIZABLE
    ...
    private void install() {
        ...
        MetaField.addDbRefreshListener(this, null, new MetaField[] { DbObject.fComponents,
                DbSemanticalObject.fName, DbBEQualifier.fIcon });
    }
    ...
    public void refreshAfterDbUpdate(DbUpdateEvent event) throws DbException {
        if (event.metaField == DbObject.fComponents) {
            if (!(event.neighbor instanceof DbBEQualifier))
                return;
            ...
            if (model != null && model == lastModel)
                ...
                || event.metaField == DbBEQualifier.fIcon) {
            if (!(event.dbo instanceof DbBEQualifier))
                return;
    ...
    private void loadListData(DbObject dbo) throws DbException {
        ...
        if (model != null) {
            DbEnumeration dbEnum = model.componentTree(DbBEQualifier.metaClass);
            while (dbEnum.hasMoreElements()) {
                DbBEQualifier qualifier = (DbBEQualifier) dbEnum.nextElement();
                listItems.add(new DefaultComparableElement(qualifier, ApplicationContext
                        .getSemanticalModel().getDisplayText(qualifier, DbObject.SHORT_FORM, model,
                                null), qualifier.getIcon()));
            }
            dbEnum.close();
        }
