...
final class ConvertDataModelWorkMode {
    ...
    private static class ConvertDataModelWorker extends Worker {
        ...
        /*
         * Conversion des mod�les relationnels en mod�les conceptuels
         */
        private void convertORRelationship(DbORDataModel model) throws DbException {
            ...
            //les colonnes des cl�s �trang�res sont d�truites 
            //puisque les cl�s sont d�truites avec les associations d'origines
            ...
            //Les associations d'origines sont misent dans un array sinon on boucle sans fin en en cr�ant des nouvelles
            while (dbEnum.hasMoreElements()) {
            ...
            for (int i = 0; i < nb; i++) {
                ...
                // cr�er une table d'association et deux arcs pour remplacer la DbORAssociation d'origine  
                ...
                //transf�rer les d�pendances sur les r�les
        ...
        /*
         * Conversion des mod�les conceptuels en mod�les relationnels
         */
            ...
            while (dbEnum.hasMoreElements()) {
                ...
                if (assocTable.isIsAssociation()) {
                    ...
                    //Relation ternaire et plus
                        ...
                        //Association non li�e ou li�e incorrectement
                        //??? est-ce qu'on ne devrait pas d�truire la table d'association ?
        ...
        /*
         * Conversion des relations conceptuels BINAIRES dans un mod�le relationnel
         */
        private void convertBinaryRelationship(DbORTable assocTable) throws DbException {
            ...
            if (m_absorptionToDo) {
                ...
                //comptabilisation des d�pendances sur les arcs pour pouvoir les recr�er sur la nouvelle association
                //Pour le moment on conserve les d�pendances seulement pour les relations binaires
                DbEnumeration srcDepEnum = srcEnd.getDependentConstraints().elements();
                while (srcDepEnum.hasMoreElements()) {
                ...
                //D�placer les colonnes des asociations vers la table enfant
                ...
                if (assocTabColumnsList.size() > 0) {
                    ...
                    for (int i = 0; i < dbColumns.length; i++) {
                        ...
                        getController().println(msg);
                        //Les attributs absorb�s dans une table optionnelle (0,1) ne peuvent �tre NON NULL
                ...
                /*
                 * d�pendance sur cl� unique sur le r�le enfant version 2.3 build 504 : il a �t�
                 * d�cid� de ne pas g�n�rer de cl� alternative lors de la conversion selon les
                 * anciennes r�gles de conversion mais seulement de propager les d�pendances
                 * existantes
                 */

                //Build 507 : on ne transfert les d�pendances sur le r�le enfant 
                //que si la multiplicit� est 1,1 selon les r�gles d'int�grit�
                ...
                //Cr�er les nouvelles associations graphiques
                ...
                while (goEnum.hasMoreElements()) {
                    ...
                    DbSMSDiagram diagram = (DbSMSDiagram) assocTableGo
                            .getCompositeOfType(DbSMSDiagram.metaClass);
                    //trouver les 2 tableGo associ�es
                    ...
                    //Les 2 arcs doivent �tre pr�sents
                ...
                //TODO  la navigabilit� est d�fini par d�faut. On devrait assigner celle des arcs           
                //orAssociation.setIsIdentifying()TODO assigner la bonne multiplicit�
                ...
                // Suppression obligatoires des d�pendances existantes puisque de nouvelles sont cr��es pour la table d'intersection
                // ces d�pendances �taient de toute fa�on erron�es selon les r�gles d'int�grit�s
                DbEnumeration srcDepEnum = srcEnd.getDependentConstraints().elements();
                while (srcDepEnum.hasMoreElements()) {
                    key = (DbORPrimaryUnique) srcDepEnum.nextElement();
                ...
                // les arcEnds(backEnds) devenus les associationEnd de la table d'intersection sont modifi�s
                ...
                // Cr�ation des d�pendances sur les r�les
                ...
                // Les deux optionnels (0,1)
                    ...
                    getController().println(msg);
                    // version 2.3 build 504 : il a �t� d�cid� de ne pas g�n�rer de cl� alternative lors de la conversion 
                    // selon les anciennes r�gles de conversion.
                    /*
        ...
        /*
         * Conversion des relations conceptuels TERNAIRES et plus en mod�les relationnels
         */
        private void convertTernaryAndMoreRelationship(DbORTable assocTable) throws DbException {
            ...
            if (m_absorptionToDo) {// Au moins une multiplicit� 1,1 ou 0,1 est pr�sente
                // NOTE : Pour le moment on conserve les d�pendances seulement pour les relations binaires
                // Les cas sp�cifiques qui pourraient d�roger de la r�gle d'int�grit� qui dit qu'une d�pendance 
                // ne peut exister dans une relation ternaire pourront �tre analyser dans une autre version.

                //D�terminer la table enfant
                for (int i = 0; i < dboEnds.length; i++) {
                    assocEnd = (DbORAssociationEnd) dboEnds[i];
                    endMult = assocEnd.getMultiplicity();
                    // Ici on ne v�rifie pas si la table enfant est navigable. C'est moins significatif 
                    //parce que de toute fa�on le r�sultat converti(absorb� ou non) red�fini tous les r�les 
                    //et la navigabillt� est alors assign�e correctement.
                ...
                //Cr�er les nouvelles associations s�mantiques entre la table enfant et chaque table parent
                ...
                for (int i = 0, idx = 1; i < dboEnds.length; i++) {
                    ...
                    // Non transfert des d�pendances existantes selon les r�gles d'int�grit�s
                    ...
                    //D�terminer si ce r�le enfant aura une d�pendance primaire 
                    // mais seulement s'il est EXACTLY_ONE
                    //Build 508 : Il a �t� d�cid� qu'aucune d�pendance ne serait cr��e pour les relations ternaires 
                    //dans laquelle il y aurait un parent 1,1 ou 0,1. Ainsi on applique la m�me r�gle que si 
                    // c'�taient plusieurs relations binaires
                    /*
                ...
                // Appliquer la d�pendance primaire s'il y a lieu 
                //Build 508 : Il a �t� d�cid� qu'aucune d�pendance ne serait cr��e pour les relations ternaires 
                //dans laquelle il y aurait un parent 1,1 ou 0,1. Ainsi on applique la m�me r�gle que si 
                // c'�taient plusieurs relations binaires
                /*
                ...
                while (childgoEnum.hasMoreElements()) {
                    ...
                    //LA TABLE D'ASSOCIATION GRAPHIQUE repr�sentant la s�mantique courante (assocTable)
                    //via la liste des arcs li�s a la table enfant 
                    ...
                    //LES TABLES GRAPHIQUES � lier � la table enfant 
                    //via la liste des arcs li�s � la table d'association graphique trouv�e
                ...
                if (m_allParentMaxN) {
                    ...
                    getController().println(wMsgPKDependAll);
                } else {//Au moins une multiplicit� 0,1 existe
                    DbObject[] assocEnds = new DbObject[assocTableEndList.size()];
                    for (int i = 0; i < assocEnds.length; i++) {
                        assocEnds[i] = (DbObject) assocTableEndList.get(i);
        ...
        /*
         * evalue si l'option d'absorption peut s'appliquer selon les multiplicit�s de la relation
         * ternaire et plus
         */
        ...
        private boolean isBinaryAbsorptionNeeded(SMSMultiplicity srcMult, SMSMultiplicity dstMult)
            ...
            if ((srcMult.isMaxN()) && (dstMult.isMaxN()))
                return false;
            //Les deux minimales 1 (le cas 1,N---1,N est trait� par la condition pr�c�dente)
        ...
        /*
         * retourne le nom � afficher dans les commentaires du worker (entre autre)
         */
        ...
        /*
         * Inverse les valeurs des r�les lorsque non absorption
         */
        ...
        /*
         * Copie les valeurs des r�les lorsque absorption
         */
