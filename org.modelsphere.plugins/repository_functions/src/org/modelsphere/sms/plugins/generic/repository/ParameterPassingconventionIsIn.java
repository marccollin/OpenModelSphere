...
/**
 * For a parameter, tells whether its passing convention is IN. <br>
 ...
 * 
 * @deprecated
 * @see org.modelsphere.sms.templates.GenericMapping#OrparameterPassingconvention.<br>
 */
public final class ParameterPassingconventionIsIn extends UserDefinedField {
    private static final PluginSignature signature = new PluginSignature(
            "ParameterPassingconventionIsIn", "$Revision: 4 $",
            ApplicationContext.APPLICATION_AUTHOR, "$Date: 2009/04/14 14:00p $", 212); // NOT LOCALIZABLE

    public ParameterPassingconventionIsIn() {
    } //Parameter-less constructor required by jack.io.Plugins

    public ParameterPassingconventionIsIn(String ruleName, String subrule, Modifier[] modifiers)
    ...
    public boolean expand(Writer output, Serializable object, Rule.RuleOptions options)
        ...
        String value;
        DbORParameter parameter = (DbORParameter) object;

        try {
            if (parameter.getPassingConvention().getValue() == SMSPassingConvention.IN)
...
} //end of ParameterPassingconventionIsIn
